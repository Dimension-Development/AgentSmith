...
- [x] `app/(auth)/login/page.tsx`: pass `shouldCreateUser: false` in `signInWithOtp`
- [x] Hosted Supabase: **Disable new user signups** in Auth settings (document the
      toggle in `docs/database.md` deploy section)
- [x] Google OAuth: disabling signups covers it (sign-in only for existing users) —
      verify and document
- [x] Decide + document the invite mechanism (MVP: admin creates users in the Supabase
      dashboard; that's fine, just write it down)
- [x] Seed admin (`admin@agentsmith.local` / `admin123`): assert it is **local-only**
...
- [x] New migration is unnecessary — the insert is idempotent (`on conflict do nothing`)
      and already applied locally; instead move the insert into `supabase/seed.sql` for
      fresh local resets
- [x] Remove the insert from the initial migration **only if** we accept re-baselining
      before first hosted push (we haven't pushed hosted yet — safe now, impossible
      later; do it now)
- [x] Replace `ensureDefaultProject()` hard-coded GitHub binding with a neutral default
...
- [x] 500 path: log full error server-side, return `{ error: "Internal server error" }`
- [x] Keep `ServiceError` and Zod messages as-is (they're intentional)

## 4. Environment & config hygiene

- [x] Verify hosted env vars documented: `NEXT_PUBLIC_SUPABASE_URL`,
      `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (server-only)
- [x] Confirm `SUPABASE_SERVICE_ROLE_KEY` is never imported client-side
      (`lib/supabase/admin.ts` is server-only today — add a `import "server-only"`
      guard to make the bundler enforce it)
- [x] `.mcp.json` contains a live `asm_` key — already gitignored; add a
      `.mcp.json.example` with placeholder so the setup is copyable
- [x] Auth redirect URLs: add the hosted domain to Supabase allowed redirect list;
      document in `docs/database.md`

## 5. Basic abuse limits (cheap, do now; real rate limiting later)

- [x] Middleware currently runs `supabase.auth.getUser()` on every API request even for
...
- [x] Note (not MVP): per-key rate limiting once hosted usage exists — track in Plan 06
